<script>
    $( ".editpage" ).on("click",function(e){
        ...
        var arr = id.split('__');
        zoneTypeId = arr[1];
        getZoneTypeForm(zoneTypeId);
    });

</script>

<div class="widget-body  <?php echo ((!empty($ZONE_TYPES) && (isset($ZONE_TYPES['results']) && count($ZONE_TYPES['results']) > 0 )) ? 'no-padding' : 'padding') ?>">

    <table class="table table-striped table-bordered table-hover" width="100%">
        <?php
            if (!empty($ZONE_TYPES) && (isset($ZONE_TYPES['results']) && count($ZONE_TYPES['results']) > 0 )) {
        ?>
        <thead>			                
            <tr>
                <th data-class="expand">Zone Type</th>
                <th>Slug</th>
                <th></th>
            </tr>
        ...
        <tbody>
            <?php
                foreach ($ZONE_TYPES['results'] as $zone_type) {
                    ?>  
                    <tr>
                        <td><?php echo $zone_type->zone_type ?></td>
                        <td><?php echo $zone_type->zone_slug ?></td>
                        <td><a href="#" id="zone__<?php echo $zone_type->zone_type_id; ?>" class="editpage"><i class="fa fa-edit fa-2x"></i></a> &nbsp;
                            <a id="delete__<?php echo $zone_type->zone_type_id ?>" href="#" class="openModal"> <i class="fa fa fa-trash-o fa-2x"></i></a>
                        </td>
                    </tr>
                <?php
                }
        ...
        <input type="hidden" name="list_zone_type_id" id="list_zone_type_id" value="">
    </table>
    <div class="">
        <div >
            <div>
            <?php
                    $uri = '';
                    buildPagination($ZONE_TYPES['param'], $uri);
            ?>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        $(".openModal").on('click', function(e) {
            ...
            $("#delete_yes").on('click', function(e) {
                id = $('#list_zone_type_id').val();

                var postArray = {
                    zoneTypeId: id
                };
                url = '/zone/zonetype/remove';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h3>Are you sure ?</h3>
                <p>
                Please confirm you want to delete this zone type. THIS ACTION IS NOT REVERSIBLE
                </p>
            </div>
