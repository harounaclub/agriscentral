...
<div class="widget-body <?php echo ((!empty($ZONES) && (isset($ZONES['results']) && count($ZONES['results']) > 0 )) ? 'no-padding' : 'padding') ?>">

    <table class="table table-striped table-bordered table-hover" width="100%">
        <?php
            if (!empty($ZONES) && (isset($ZONES['results']) && count($ZONES['results']) > 0 )) {
        ?>
        <thead>			                
            <tr>
                <th data-class="expand">Zone Name</th>
                <th>Zone Type</th>
                <th></th>
            </tr>
        ...
        <tbody>
            <?php
                foreach ($ZONES['results'] as $zone) {
                    ?>  
                    <tr>
                        <td><?php echo $zone->zone_name ?></td>
                        <td><?php echo $zone->zone_type ?></td>
                        <td><a href="#" id="zone__<?php echo $zone->zone_id; ?>" class="editpage"><i class="fa fa-edit fa-2x"></i></a> &nbsp;
                            <a id="delete__<?php echo $zone->zone_id ?>" href="#" class="openModal"> <i class="fa fa fa-trash-o fa-2x"></i></a>
                        </td>
                    </tr>
                <?php
                }
        ...
        <input type="hidden" name="list_zone_id" id="list_zone_id" value="">
    </table>
    <div class="">
        <div >
            <div>
            <?php
                    $uri = '';
                    buildPagination($ZONES['param'], $uri);
            ?>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h3>Are you sure ?</h3>
                <p>
                Please confirm you want to delete this zone. THIS ACTION IS NOT REVERSIBLE
                </p>
            </div>
